...
export default function Content() {
  ...
  const menuItems1 = categories.length > 0 ? [
    categories[0]?.data[5],
    categories[1]?.data[3],
    categories[2]?.data[7],
    categories[3]?.data[4],
  ...
  const menuItems2 = categories.length > 0 ? [
    categories[4]?.data[2],
    categories[5]?.data[7],
    categories[6]?.data[0],
    categories[7]?.data[1],
  ...
  const menuItems = [...menuItems1, ...menuItems2]; // همه آیتم‌ها را در یک آرایه
  ...
  useEffect(() => {
    if (menuItems.length > 0) {
      ...
      return () => clearInterval(intervalId); // پاکسازی تایمر
    }
  }, [menuItems]);
  ...
  return (
    <div className={styles.contentmain}>
      <div className={styles.div1}>
        {menuItems1.length > 0 && menuItems1.map((item, index) => (
          item && (
            <animated.div
              key={item.id}
              style={springs[index]} // انیمیشن از چپ به راست
              className={`${styles.menu2} ${selectedItem.id === item.id ? styles.clicked : ''}`}
              onClick={() => {
                setSelectedItem(item);
                handleNavigate(categories[index]);
              }}
            >
              <div className={styles.btntextM}>
                <h5>{item.description}</h5>
              </div>
              <img src={item.imageUrl} className={styles.img2} alt={item.name} />
            </animated.div>
          )
        ))}
      </div>
      ...
      <div className={styles.div3}>
        {menuItems2.length > 0 && menuItems2.map((item, index) => (
          item && (
            <animated.div
              key={item.id}
              style={springs[index + menuItems1.length]} // انیمیشن از راست به چپ
              className={`${styles.menu1} ${selectedItem.id === item.id ? styles.clicked : ''}`}
              onClick={() => {
                setSelectedItem(item);
                handleNavigate(categories[index + menuItems1.length]);
              }}
            >
              <div className={styles.btntextM}>
                <h5>{item.description}</h5>
